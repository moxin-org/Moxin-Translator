            ...
            if let Some(mut list) = item.as_portal_list().borrow_mut() {
                ...
                while let Some(item_id) = list.next_visible_item(cx) {
                    if item_id < self.filtered_voices.len() {
                        ...
                        let can_preview =
                            voice.preview_audio.is_some() || voice.reference_audio_path.is_some();
                        ...
                        self.item_areas
                            .push((item_id, item_area, preview_area, delete_area));
                    }
                }
            }
        }
...
impl VoiceSelector {
    ...
    fn handle_filter_button_clicks(&mut self, cx: &mut Cx, actions: &ActionsBuf) {
        ...
        if self
            .view
            .button(ids!(
                filter_bar.category_row.category_filter_group.filter_all_btn
            ))
            .clicked(actions)
        {
        ...
        if self
            .view
            .button(ids!(
                filter_bar.category_row.language_filter_group.lang_all_btn
            ))
            .clicked(actions)
        {
        ...
        if self
            .view
            .button(ids!(
                filter_bar.category_row.language_filter_group.lang_zh_btn
            ))
            .clicked(actions)
        {
        ...
        if self
            .view
            .button(ids!(
                filter_bar.category_row.language_filter_group.lang_en_btn
            ))
            .clicked(actions)
        {
    ...
    fn update_category_filter_buttons(&mut self, cx: &mut Cx) {
        let all_active = if self.category_filter == VoiceFilter::All {
            1.0
        } else {
            0.0
        };
        let male_active = if self.category_filter == VoiceFilter::Male {
            1.0
        } else {
            0.0
        };
        let female_active = if self.category_filter == VoiceFilter::Female {
            1.0
        } else {
            0.0
        };
        let character_active = if self.category_filter == VoiceFilter::Character {
            1.0
        } else {
            0.0
        };
    ...
    fn update_language_filter_buttons(&mut self, cx: &mut Cx) {
        let all_active = if self.language_filter == LanguageFilter::All {
            1.0
        } else {
            0.0
        };
        let zh_active = if self.language_filter == LanguageFilter::Chinese {
            1.0
        } else {
            0.0
        };
        let en_active = if self.language_filter == LanguageFilter::English {
            1.0
        } else {
            0.0
        };
