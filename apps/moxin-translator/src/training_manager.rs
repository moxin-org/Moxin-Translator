...
impl TrainingManager {
        ...
        let mut child = match backend {
            ...
            TrainingBackend::OptionB => {
                let trainer_bin = match Self::resolve_rust_trainer_bin() {
                    ...
                    Err(e) => {
                        let mut prog = progress.lock();
                        prog.status = TrainingStatus::Failed { error: e.clone() };
                        prog.log_lines.push(format!("[ERROR] {}", e));
                        prog.last_updated = Instant::now();
                        return None;
                    }
    ...
    fn resolve_rust_trainer_bin() -> Result<PathBuf, String> {
        ...
        let workspace_candidate = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../target/release/moxin-fewshot-trainer");
        ...
        Err("Cannot find moxin-fewshot-trainer binary. Set MOXIN_FEWSHOT_TRAINER_BIN to a valid path."
            .to_string())
    }
    ...
    fn handle_event(json_line: &str, progress: &Arc<Mutex<TrainingProgress>>) {
        ...
        match event.event_type.as_str() {
            "STAGE" => {
                ...
                prog.log_lines.push(format!("[STAGE] {}", event.message));
            ...
            "INFO" | "LOG" => {
                prog.log_lines.push(format!("[INFO] {}", event.message));
            }

            "WARNING" => {
                prog.log_lines.push(format!("[WARNING] {}", event.message));
            ...
            "ERROR" => {
                ...
                prog.log_lines.push(format!("[ERROR] {}", event.message));
