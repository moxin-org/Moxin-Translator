...
impl UpdateConfig {
    fn from_env() -> Self {
        Self {
            ...
            cache_dir: read_env_path(CACHE_DIR_ENV).unwrap_or_else(default_update_cache_dir),
...
fn cleanup_cached_installers_in_dir(
    cache_dir: &Path,
...
mod tests {
    use super::{
        check_and_prepare_update_with_config, cleanup_cached_installers_in_dir, is_newer_version,
        launch_update_installer_with_config, normalize_version, parse_version, CheckOutcome,
        PreparedUpdate, UpdateConfig,
    };
    use std::fs;
    use std::path::{Path, PathBuf};
    ...
    fn check_and_prepare_update_downloads_and_reuses_cached_installer() {
        ...
        let config = test_config(
            format!("file://{}", release_json.display()),
            cache_dir.path(),
        );
