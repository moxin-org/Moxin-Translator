    ...
    pub fn poll(&mut self) -> Option<ExecutorEvent> {
        ...
        match &progress.status {
            TrainingStatus::Completed {
                ...
                let _ =
                    task_persistence::update_task_progress(&task_id, 1.0, 8, "Training completed");
                ...
                log::info!(
                    "[Executor] Task {} completed — gpt={} sovits={} ref={}",
                    task_id,
                    gpt.display(),
                    sovits.display(),
                    ref_audio.display()
            ...
            TrainingStatus::Running => {
                ...
                if should_update {
                    ...
                    let sub_step = if progress.sub_step > 0 {
                        Some(progress.sub_step as u32)
                    } else {
                        None
                    };
                    let sub_total = if progress.sub_total > 0 {
                        Some(progress.sub_total as u32)
                    } else {
                        None
                    };
