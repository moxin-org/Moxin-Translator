    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        match event {
            ...
            Event::Drop(de) => {
                ...
                let dropped_path: Option<std::path::PathBuf> = de
                    .items
                    .iter()
                    .find(|item| {
        ...
        if self.recording_status == RecordingStatus::Transcribing {
            ...
            // Calculate rotation based on elapsed time (1 full rotation per 2 seconds)
            let elapsed = self
                .transcription_animation_start_time
                .unwrap()
                .elapsed()
                .as_secs_f64();
            let rotation = (elapsed * 0.5) % 1.0; // 0.5 rotations per second
...
impl VoiceCloneModal {
                        ...
                        if let Ok(modified) = metadata.modified() {
                            if modified < threshold {
                                ...
                                if let Err(e) = fs::remove_file(&path) {
                                    eprintln!(
                                        "[VoiceClone] Failed to remove old temp file {:?}: {}",
                                        path, e
                                    );
                                } else {
    ...
    fn handle_file_selected(&mut self, cx: &mut Cx, path: PathBuf) {
        ...
        match voice_persistence::validate_audio_file(&path) {
            Ok(info) => {
                ...
                let express_max_secs: f32 = if Self::is_qwen3_zero_shot() {
                    8.0
                } else {
                    10.0
                };
        ...
        if duration > express_max_secs {
            self.add_log(
                ...
                &format!(
                    ...
                    express_max_secs, express_max_secs
                ),
            );
        }
    ...
    fn resample(samples: &[f32], source_rate: u32, target_rate: u32) -> Vec<f32> {
        ...
        match resampler.process(&input_frames, None) {
            ...
            Err(e) => {
                eprintln!(
                    "[VoiceClone] Resampling error: {}, falling back to linear interpolation",
                    e
                );
    ...
    fn start_training_recording(&mut self, cx: &mut Cx) {
        ...
        self.add_training_log(
            cx,
            "[INFO] Speak clearly with varied sentences for best training results",
        );
        ...
        std::thread::spawn(move || {
            ...
            *sample_rate_store.lock() = sample_rate;
            ...
            eprintln!(
                "[TrainingRec] Recording started ({}Hz, {} channels)",
                sample_rate, channels
            );
    ...
    fn handle_training_file_selected(&mut self, cx: &mut Cx, path: PathBuf) {
        ...
        // Validate duration (minimum 10 seconds)
        if duration < 10.0 {
            let msg = format!("Audio too short: {:.1}s (minimum: 10s)", duration);
            self.add_training_log(cx, &format!("[ERROR] {}", msg));
            self.show_error(cx, &msg);
            return;
        }
    ...
    pub fn show(&self, cx: &mut Cx) {
        ...
        if let Some(mut inner) = self.borrow_mut() {
            ...
            let asr_available = if let Some(ref shared_state) = inner.shared_dora_state {
                ...
                if has_asr {
                    if has_audio_input {
                        inner.add_log(cx, "[INFO] ASR ready (recording + transcription available)");
                    } else {
                        inner.add_log(
                            cx,
                            "[INFO] ASR ready (transcription only, live recording disabled)",
                        );
                    }
                    true
                } else {
